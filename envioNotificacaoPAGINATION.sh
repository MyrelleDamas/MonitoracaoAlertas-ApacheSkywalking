...
# Função para extrair dados da página usando Puppeteer. O puppeteer é uma ferramenta que pode ser utilizada para testes de interface, testes de integração, automação de tarefas, extração de dados e muitas outras atividades que requerem a interação com um navegador.
...
(async () => {
    ...
    do {
        ...
        pageData = pageData.concat(dataElements);

        nextPageButton = await page.$('.pagination .el-pagination__next > .btn');
        if (nextPageButton) {
            await nextPageButton.click();
            await page.waitForTimeout(5000);  // Aguarde o carregamento da próxima página
        }
